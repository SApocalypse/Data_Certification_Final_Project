...
def main():
    ...
    # 1. Bar graph of unique values in 'Orbit' with corresponding % of unique value 1 in 'Class' (Landing Class)
    orbit_success_pct = df.groupby('Orbit')['Class'].mean() * 100

    # Plot the bar graph
    plt.figure(figsize=(10, 6))
    orbit_success_pct.plot(kind='bar', color='skyblue')
    plt.title('Percentage of Successful Landings (Class = 1) by Orbit')
    plt.xlabel('Orbit')
    plt.ylabel('Percentage of Successful Landings (Class = 1)')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.show()

    # Plotting with seaborn catplot for additional visualizations
    ...
    features_one_hot = pd.get_dummies(df, columns=['Orbit', 'LaunchSite', 'LandingPad', 'Serial', 'Outcome',
                                                   'BoosterVersion', 'GridFins', 'Reused', 'Legs'])
